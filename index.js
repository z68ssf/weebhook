...
// =============================================
//         إعدادات عامة — عدّل هنا فقط
// =============================================
const BOT_OWNER_ID = '1224722940701048927';   // ID حسابك
const BOT_TOKEN    = 'MTUxMTUwOTk2MDMzNTQyNTYyNg.GPAvb7.3LM1mxx2hnPLd3H-Gv3axhPS39w6Rv6zBYGUAw';
const CLIENT_ID    = '1511509960335425626'; // ID البوت من Developer Portal
const LOG_CHANNEL_ID = '1513253796598255828'; // ID روم اللوق
...
function isWhitelisted(userId) {
  return userId === BOT_OWNER_ID || whitelist.includes(userId);
}
...
async function sendLog({ type, executor, violation, punishment, extra = [], color = 0xE24B4A }) {
  try {
    ...
    const titles = {
      ban:        'تبنيد عضو',
      serverEdit: 'تغيير إعدادات السيرفر',
      adminRole:  'إعطاء صلاحية Administrator',
      channelDel: 'حذف روم',
      roleDel:    'حذف رتبة',
      botAdd:     'إضافة بوت غير مصرح',
      whitelist:  'تعديل الوايت ليست',
      webhook:    'إرسال ويبهوك',
    };
...
async function punish(guild, userId, reason) {
  try {
    ...
    await sendLog({
      ...
      executor: `<@${userId}>`,
      violation: reason,
      punishment: `${E.ghost} بان <a:by_ez_181:1470778978196521105>`,
      color: 0xE24B4A,
    });
  } catch (err) {
    console.error(`❌ فشل البان: ${err.message}`);
  }
}
...
async function registerCommands() {
  const commands = [
    new SlashCommandBuilder()
      .setName('whitelist')
      .setDescription('إدارة الوايت ليست')
      .addSubcommand(sub =>
        sub.setName('add')
          .setDescription('أضف شخص للوايت ليست')
          .addUserOption(opt =>
            opt.setName('user').setDescription('الشخص').setRequired(true)))
      .addSubcommand(sub =>
        sub.setName('remove')
          .setDescription('أزل شخص من الوايت ليست')
          .addUserOption(opt =>
            opt.setName('user').setDescription('الشخص').setRequired(true)))
      .addSubcommand(sub =>
        sub.setName('list')
          .setDescription('عرض الوايت ليست'))
      .toJSON(),

    new SlashCommandBuilder()
      .setName('protection')
      .setDescription('إدارة حمايات البوت')
      .addSubcommand(sub =>
        sub.setName('status')
          .setDescription('عرض حالة الحمايات'))
      .addSubcommand(sub =>
        sub.setName('toggle')
          .setDescription('تفعيل أو تعطيل حماية')
          .addStringOption(opt =>
            opt.setName('type')
    ...
    new SlashCommandBuilder()
      .setName('webhooks')
      .setDescription('<:by_ez_135:1491518839429529813>عرض كل الويبهوكات في السيرفر')
      .toJSON(),
  ];
...
client.on(Events.GuildUpdate, async (oldGuild, newGuild) => {
  ...
  if (!executor) return;
  if (executor.id === client.user.id || isWhitelisted(executor.id)) return;

  await sendLog({
    type: 'serverEdit',
    executor: `<@${executor.id}>`,
    violation: '<:by_ez_135:1491518839429529813>تغيير إعدادات السيرفر بدون صلاحية',
    punishment: `${E.ghost} بان `,
    color: 0xE24B4A,
  });
...
client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
  ...
  await sendLog({
    type: 'adminRole',
    executor: `<@${executor.id}>`,
    violation: `إعطاء رتبة **${dangerRole.name}** لـ <@${newMember.id}> وهي تحتوي على Administrator`,
    punishment: `${E.ghost} بان دائم + سحب الرتبة`,
    color: 0xE24B4A,
  });
...
client.on(Events.GuildRoleUpdate, async (oldRole, newRole) => {
  ...
  await sendLog({
    type: 'adminRole',
    executor: `<@${executor.id}>`,
    violation: `إضافة صلاحية Administrator لرتبة **${newRole.name}**`,
    punishment: `${E.ghost} بان  + استعادة صلاحيات الرتبة`,
    color: 0xE24B4A,
  });
...
client.on(Events.ChannelDelete, async (channel) => {
  ...
  await sendLog({
    type: 'channelDel',
    executor: `<@${executor.id}>`,
    violation: `حذف روم **${channel.name}** — العداد: ${count}/${LIMITS.channelDeletes}`,
    punishment: count >= LIMITS.channelDeletes ? `${E.ghost} بان ` : `⚠️ تحذير — ${LIMITS.channelDeletes - count} متبقية`,
    color: count >= LIMITS.channelDeletes ? 0xE24B4A : 0xFAA61A,
  });

  if (count >= LIMITS.channelDeletes)
    await punish(channel.guild, executor.id, `تجاوز حد حذف الرومات (${LIMITS.channelDeletes}/يوم)`);
});
...
client.on(Events.GuildRoleDelete, async (role) => {
  ...
  await sendLog({
    type: 'roleDel',
    executor: `<@${executor.id}>`,
    violation: `حذف رتبة **${role.name}** — العداد: ${count}/${LIMITS.roleDeletes}`,
    punishment: count >= LIMITS.roleDeletes ? `${E.ghost} بان دائم` : `⚠️ تحذير — ${LIMITS.roleDeletes - count} متبقية`,
    color: count >= LIMITS.roleDeletes ? 0xE24B4A : 0xFAA61A,
  });

  if (count >= LIMITS.roleDeletes)
    await punish(role.guild, executor.id, `تجاوز حد حذف الرولات (${LIMITS.roleDeletes}/يوم)`);
});
...
client.on(Events.GuildAuditLogEntryCreate, async (entry, guild) => {
  ...
  await sendLog({
    ...
    punishment: count >= LIMITS.bans ? `${E.ghost} بان ` : `⚠️ تحذير — ${LIMITS.bans - count} متبقية`,
    color: count >= LIMITS.bans ? 0xE24B4A : 0xFAA61A,
  });

  if (count >= LIMITS.bans)
    await punish(guild, executor.id, `تجاوز حد البان (${LIMITS.bans}/يوم)`);
});
...
client.on(Events.GuildMemberAdd, async (member) => {
  ...
  await sendLog({
    type: 'botAdd',
    executor: executor ? `<@${executor.id}>` : 'غير معروف',
    violation: `إضافة بوت <@${member.id}> بدون إذن`,
    punishment: `${E.ghost} بان البوت + بان المضيف`,
    color: 0xE24B4A,
  });

  await punish(member.guild, member.id, 'إضافة بوت غير مصرح');
  if (executor && !isWhitelisted(executor.id))
    await punish(member.guild, executor.id, 'إضافة بوت غير مصرح');
});
    ...
    if (interaction.user.id !== BOT_OWNER_ID) {
      await interaction.reply({
        embeds: [new EmbedBuilder()
          .setColor(0xE24B4A)
          .setAuthor({ name: `${E.ghost}  خطأ في الصلاحيات`, iconURL: client.user.displayAvatarURL() })
          .setDescription('> هذا الأمر مخصص للمالك فقط.')
          .setTimestamp()
          .setFooter({ text: 'by zwh.', iconURL: client.user.displayAvatarURL() })],
      });
      return false;
    ...
    try {
      ...
      if (!allWebhooks.size) {
        return interaction.editReply({
          embeds: [new EmbedBuilder()
            .setColor(0x5865F2)
            .setAuthor({ name: `${E.ghost}  الويبهوكات`, iconURL: client.user.displayAvatarURL() })
            .setDescription('> لا يوجد ويبهوكات في السيرفر.')
            .setTimestamp()
            .setFooter({ text: 'by zwh.', iconURL: client.user.displayAvatarURL() })],
        });
      }
      ...
      const lines = allWebhooks.map(w => {
        const channel = w.channelId ? `<#${w.channelId}>` : 'غير معروف';
        const owner   = w.owner ? `<@${w.owner.id}>` : 'غير معروف';
        const isMine  = w.owner?.id === client.user.id ? ` ${E.ghost}` : '';
        return `**${w.name}${isMine}**\n> ${E.dot} الروم: ${channel}\n> ${E.dot} المنشئ: ${owner}\n> ${E.dot} \`${w.id}\``;
      });
      ...
      const embeds = chunks.map((chunk, i) =>
        new EmbedBuilder()
          .setColor(0x5865F2)
          .setAuthor(i === 0
            ? { name: `${E.ghost}  الويبهوكات (${allWebhooks.size})`, iconURL: client.user.displayAvatarURL() }
            : { name: '​' })
      ...
      return interaction.editReply({
        embeds: [new EmbedBuilder().setColor(0xE24B4A).setDescription(`❌ فشل جلب الويبهوكات: ${err.message}`)],
      });
    }
  }
  ...
  if (interaction.commandName === 'protection') {
    ...
    if (sub === 'status') {
      return interaction.reply({
        embeds: [new EmbedBuilder()
          .setColor(0x5865F2)
          .setAuthor({ name: `${E.ghost}  حالة الحمايات`, iconURL: client.user.displayAvatarURL() })
          .addFields([
    ...
    if (sub === 'toggle') {
      ...
      return interaction.reply({
        embeds: [new EmbedBuilder()
          .setColor(enabled ? 0x57c97a : 0xE24B4A)
          .setAuthor({ name: `${E.ghost}  تم ${enabled ? 'تفعيل' : 'تعطيل'} الحماية`, iconURL: client.user.displayAvatarURL() })
          .addFields([
    ...
    if (sub === 'add') {
      ...
      if (whitelist.includes(target.id)) {
        return interaction.reply({
          embeds: [new EmbedBuilder()
            .setColor(0xFAA61A)
            .setAuthor({ name: `${E.ghost}  تنبيه`, iconURL: client.user.displayAvatarURL() })
            .setDescription(`> **${target.tag}** موجود في الوايت ليست بالفعل.`)
            .setTimestamp()
            .setFooter({ text: 'by zwh.', iconURL: client.user.displayAvatarURL() })],
        });
      }
      ...
      return interaction.reply({
        embeds: [new EmbedBuilder()
          .setColor(0x57c97a)
          .setAuthor({ name: `${E.ghost}  تم بنجاح`, iconURL: client.user.displayAvatarURL() })
          .setDescription(`> تمت إضافة **${target.tag}** للوايت ليست.`)
          .setTimestamp()
          .setFooter({ text: 'by zwh.', iconURL: client.user.displayAvatarURL() })],
      });
    }
    ...
    if (sub === 'remove') {
      ...
      if (!whitelist.includes(target.id)) {
        return interaction.reply({
          embeds: [new EmbedBuilder()
            .setColor(0xFAA61A)
            .setAuthor({ name: `${E.ghost}  تنبيه`, iconURL: client.user.displayAvatarURL() })
            .setDescription(`> **${target.tag}** غير موجود في الوايت ليست.`)
            .setTimestamp()
            .setFooter({ text: 'by zwh.', iconURL: client.user.displayAvatarURL() })],
        });
      }
      ...
      return interaction.reply({
        embeds: [new EmbedBuilder()
          .setColor(0x57c97a)
          .setAuthor({ name: `${E.ghost}  تم بنجاح`, iconURL: client.user.displayAvatarURL() })
          .setDescription(`> تمت إزالة **${target.tag}** من الوايت ليست.`)
          .setTimestamp()
          .setFooter({ text: 'by zwh.', iconURL: client.user.displayAvatarURL() })],
      });
    }
    ...
    if (sub === 'list') {
      if (!whitelist.length) {
        return interaction.reply({
          embeds: [new EmbedBuilder()
            .setColor(0x5865F2)
            .setAuthor({ name: `${E.ghost}  الوايت ليست`, iconURL: client.user.displayAvatarURL() })
            .setDescription('> الوايت ليست فارغة حالياً.')
            .setTimestamp()
            .setFooter({ text: 'by zwh.', iconURL: client.user.displayAvatarURL() })],
        });
      }
      ...
      return interaction.reply({
        embeds: [new EmbedBuilder()
          ...
          .setDescription(list)
          .setTimestamp()
          .setFooter({ text: `by zwh. • إجمالي: ${whitelist.length} شخص`, iconURL: client.user.displayAvatarURL() })],
      });
    }
  }
});
...
async function setupWebhook(channel, config, guildIconURL) {
  try {
    ...
    if (!hook) {
      hook = await channel.createWebhook({ name: config.webhookName, avatar: guildIconURL || undefined });
      console.log(`✅ تم إنشاء ويبهوك في #${channel.name}`);
    } else {
      await hook.edit({ avatar: guildIconURL || undefined });
    }
    return hook;
  } catch (err) {
    console.error(`❌ فشل الويبهوك في #${channel.name}: ${err.message}`);
    return null;
  }
}
...
client.once(Events.ClientReady, async () => {
  ...
  console.log('\n<a:settings:1492971639292559481> الحمايات:');
  console.log(`  ${PROTECTION.serverSettings ? '✅' : '❌'} إعدادات السيرفر <a:settings:1492971639292559481> `);
  console.log(`  ${PROTECTION.antiRaid       ? '✅' : '❌'} ضد الريد — بان:${LIMITS.bans} | رومات:${LIMITS.channelDeletes} | رولات:${LIMITS.roleDeletes}`);
  console.log(`  ${PROTECTION.antiBots       ? '✅' : '❌'} ضد البوتات`);
  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
});

http.createServer((req, res) => { res.writeHead(200); res.end('البوت شغّال ✅'); })
